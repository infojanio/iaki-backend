...
describe('Get User Cashback Balance Use Case', () => {
  ...
  it('Deve calcular o saldo total de cashback do usuário', async () => {
    cashbacksRepository.items.push(
      { id: '1', user_id: 'user-1', order_id: 'order-1', amount: 50, credited_at: new Date() },
      { id: '2', user_id: 'user-1', order_id: 'order-2', amount: (-20), credited_at: new Date() },
      { id: '3', user_id: 'user-1', order_id: 'order-3', amount: 30, credited_at: new Date() }
    );

    const { balance } = await sut.execute({ userId: 'user-1' });
    expect(balance).toBe(40); // 50 + 30 - 20 = 60
